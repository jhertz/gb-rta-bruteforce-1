...
void Memory::loadState(SaveState const &state) {
	...
	cart_.setOamDmaSrc(OamDmaSrc::OAM_DMA_SRC_OFF);
...
void Memory::oamDmaInitSetup() {
	if (ioamhram_[0x146] < 0xA0) {
		cart_.setOamDmaSrc(ioamhram_[0x146] < 0x80 ? OamDmaSrc::OAM_DMA_SRC_ROM : OamDmaSrc::OAM_DMA_SRC_VRAM);
	} else if (ioamhram_[0x146] < 0xFE - isCgb() * 0x1E) {
		cart_.setOamDmaSrc(ioamhram_[0x146] < 0xC0 ? OamDmaSrc::OAM_DMA_SRC_SRAM : OamDmaSrc::OAM_DMA_SRC_WRAM);
	} else
		cart_.setOamDmaSrc(OamDmaSrc::OAM_DMA_SRC_INVALID);
}
